...
void draw(const char *str, int left, int top) {
  ...
  for (int i = 0; str[i] !=	 0; ++i) {
    ...
    c -= 32;
    if (c<0 || c>=95) continue;
    const float in_left   = (float(c%charxnum)*8.f-0.5f) / float(fontw);
    const float in_top    = (float(c/charxnum)*8.f-0.5f) / float(fonth);
    const float in_right  = in_left + 7.5f/float(fontw);
    const float in_bottom = in_top + 7.5f/float(fonth);
    const int in_width  = 80;
    const int in_height = 80;
