...
TINLINE T fract(const T &x) { return x - floor(x); }
TINLINE T mod(const T &a, const T &b) { return a - b*floor(a / b); }
TINLINE T max(const T &a, const T &b) { return a<b ? b : a; }
TINLINE T min(const T &a, const T &b) { return a<b ? a : b; }
TINLINE T min(const T &a, const T &b, const T &c) { return min(min(a, b), c); }
TINLINE T max(const T &a, const T &b, const T &c) { return max(max(a, b), c); }
TINLINE T max(const T &a, const T &b, const T &c, const T &d) { return max(max(a, b), max(c, d)); }
TINLINE T min(const T &a, const T &b, const T &c, const T &d) { return min(min(a, b), min(c, d)); }
TINLINE T min(const T &a, const T &b, const T &c, const T &d, const T &e) { return min(min(min(a, b), min(c, d)), e); }
TINLINE T max(const T &a, const T &b, const T &c, const T &d, const T &e) { return max(max(max(a, b), max(c, d)), e); }
TINLINE T clamp(const T &x, const T &lower = T(zero), const T &upper = T(one)) { return max(lower, min(x, upper)); }
TINLINE T deg2rad(const T &x) { return x * T(1.74532925199432957692e-2); }
TINLINE T rad2deg(const T &x) { return x * T(5.72957795130823208768e1); }
TINLINE T sin2cos(const T &x) { return sqrt(max(T(zero), T(one) - x*x)); }
TINLINE T cos2sin(const T &x) { return sin2cos(x); }
