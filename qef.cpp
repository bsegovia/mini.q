/*-------------------------------------------------------------------------
 ...
 - qef.cpp -> implements quadratic error function
...
static void compute_schur(double a1, double a2, double a3, double *c, double *s) {
...
static void qrstep_cols2(
  ...
  if (tau_u[0] == 0.0) {
    ...
    // perform transposed Givens rotation on B
    // multiplied by X = [ 0 1 ]
    compute_givens(tau_v[0], tau_u[1], &c, &s);
  ...
  // eliminate off-diagonal element in [ tau_u0  tau_v0 ]
    ...
    // perform Givens rotation on B
    compute_givens(tau_u[0], tau_v[0], &c, &s);
  ...
  // make colums orthogonal,
    ...
    compute_schur(tau_u[0], tau_v[0], tau_u[1], &c, &s);
  ...
  for (int i = 0; i < 3; ++i) {
    ...
    if (i + 1 < 3) {
      ...
      if (tau != 0.0) {
        for (int x = i + 1; x < 3; ++x) {
          double wx = mat[i][x];
          for (y = i + 1; y < rows; ++y) wx += mat[y][x] * (*ptrs[y - i]);
          double tau_wx = tau * wx;
          mat[i][x] -= tau_wx;
          for (y = i + 1; y < rows; ++y) mat[y][x] -= tau_wx * (*ptrs[y - i]);
        }
      }
      ...
      if (tau != 0.0) {
        for (y = i + 1; y < rows; ++y) {
          double wy = mat[y][i + 1];
          if (i == 0) wy += mat[y][i + 2] * (*ptrs[1]);
          double tau_wy = tau * wy;
          mat[y][i + 1] -= tau_wy;
          if (i == 0) mat[y][i + 2] -= tau_wy * (*ptrs[1]);
        }
      }
    }  // if (i + 1 < 3)
  }
    ...
    int rows)
{
  ...
  for (i = 1; i >= 0; --i) {
    ...
    if (tau != 0.0) {
      for (int x = i + 1; x < 3; ++x) {
        double wx = v[i + 1][x];
        for (y = i + 1 + 1; y < 3; ++y) wx += v[y][x] * u[i][y];
        double tau_wx = tau * wx;
        v[i + 1][x] -= tau_wx;
        for (y = i + 1 + 1; y < 3; ++y) v[y][x] -= tau_wx * u[i][y];
      }
    }
  }
...
static void diagonalize(
  ...
  while (b > 0) {
    if (tau_v[b - 1] == 0.0)
      ...
      int a = b - 1;
      while (a > 0 && tau_v[a - 1] != 0.0) --a;
      int n = b - a + 1;

      double u1[MAXROWS][3];
      double v1[3][3];
      for (j = a; j <= b; ++j) {
        for (i = 0; i < rows; ++i) u1[i][j - a] = u[i][j];
        for (i = 0; i < 3; ++i) v1[i][j - a] = v[i][j];
      }

      qrstep(u1, v1, &tau_u[a], &tau_v[a], rows, n);

      for (j = a; j <= b; ++j) {
        for (i = 0; i < rows; ++i) u[i][j] = u1[i][j - a];
        for (i = 0; i < 3; ++i) v[i][j] = v1[i][j - a];
      }
...
static const double zeroes[3] = {0.0, 0.0, 0.0};
static void solveSVD(
  ...
  for (i = 0; i < rows; ++i)
    if (b[i] != 0.0) for (j = 0; j < 3; ++j) w[j] += b[i] * u[i][j];
